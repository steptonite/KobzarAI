...
Українська озвучка тягне `torch` і голоси — це кілька гігабайтів і довго. Якщо
потрібне тільки керування Ollama з меню-бару, пропусти її:

```bash
./setup.sh --no-tts
```

Озвучку можна додати колись потім — просто запустивши `./setup.sh` ще раз.

| прапорець | що робить |
|---|---|
| `--no-tts` | без української озвучки: швидка установка, тільки панель Ollama |
| `--embed` | одразу тягне **bge-m3** — модель семантичного пошуку (~1.2 ГБ) |

Ставиш під [Second Brain Kit](https://github.com/steptonite/step_secondbrain_kit)?
Тоді один рядок дає рівно те, що треба, і нічого зайвого:

```bash
./setup.sh --no-tts --embed
```

Це все. `setup.sh` сам встановить усе потрібне (зокрема Homebrew, Python і Ollama — якщо їх ще нема; може один раз запитати пароль Mac), завантажить голоси (крім `--no-tts`) і збере застосунок **KobzarAI** у Програмах.
